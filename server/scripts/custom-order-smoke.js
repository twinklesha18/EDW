...
try {
  ...
  response = await fetch(`http://127.0.0.1:5105/api/custom-orders/${customOrderId}/payment-slip`, { headers: { Cookie: customerCookie } })
  assert.equal(response.status, 200, 'The customer must be able to load their protected custom-order payment slip')
  assert.match(response.headers.get('content-type') || '', /image\/png/)
  assert.ok((await response.arrayBuffer()).byteLength > 100, 'The customer custom-order slip endpoint must return image data')
  const adminReplacement = new FormData()
  adminReplacement.set('paymentSlip', new Blob([imageBuffer], { type: 'image/jpeg' }), 'admin-restored-custom-slip.jpg')
  response = await fetch(`http://127.0.0.1:5105/api/admin/custom-orders/${customOrderId}/payment-slip`, { method: 'POST', headers: { Cookie: adminCookie }, body: adminReplacement })
  const adminReplacementBody = await response.json()
  assert.equal(response.status, 200, `Admin custom-order slip restoration failed: ${JSON.stringify(adminReplacementBody)}`)
  assert.ok(adminReplacementBody.data.paymentSlip.url, 'Restored custom-order slip must have a usable URL')
