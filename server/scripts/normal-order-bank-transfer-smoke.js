...
try {
  ...
  response = await fetch(`${base}/admin/orders/${createdOrder._id}/payment-slip`, { headers: { Cookie: adminCookie } })
  assert.equal(response.status, 200, 'Administrator must be able to load the protected payment-slip image')
  assert.match(response.headers.get('content-type') || '', /image\/png/)
  assert.ok((await response.arrayBuffer()).byteLength > 100, 'The protected payment-slip endpoint must return image data')
  response = await fetch(`${base}/orders/${createdOrder.orderNumber}/payment-slip/view`, { headers: { Cookie: customerCookie } })
  assert.equal(response.status, 200, 'The customer must be able to load their protected payment-slip image')
  assert.match(response.headers.get('content-type') || '', /image\/png/)
  assert.ok((await response.arrayBuffer()).byteLength > 100, 'The customer payment-slip endpoint must return image data')
  const adminReplacement = new FormData()
  adminReplacement.set('paymentSlip', new Blob([imageBuffer], { type: 'image/jpeg' }), 'admin-restored-slip.jpg')
  response = await fetch(`${base}/admin/orders/${createdOrder._id}/payment-slip`, { method: 'POST', headers: { Cookie: adminCookie }, body: adminReplacement })
  responseBody = await response.json()
  ...
  result = await jsonRequest(`/admin/orders/${createdOrder._id}`, { method: 'PUT', cookie: adminCookie, body: { orderStatus: 'Delivered', notes: 'Attempted edit after delivery.' } })
  assert.equal(result.status, 409, 'A delivered order must reject further administrator edits')
  assert.match(result.body.message, /locked/i)
  const lockedOrder = await Order.findById(createdOrder._id).lean()
  assert.equal(lockedOrder.orderStatus, 'Delivered')
  assert.notEqual(lockedOrder.notes, 'Attempted edit after delivery.')
  response = await fetch(`${base}/orders/${createdOrder.orderNumber}/invoice`, { headers: { Cookie: customerCookie } })
