...
import { notifyAdmins, notifySafely, notifyUser } from '../services/notificationService.js'

const reviewAdminMessage = (review, user) => `${user.firstName} ${user.lastName} submitted a ${review.rating}-star review for ${review.product.name}.`

async function notifyReviewSubmitted(review, user) {
  await notifySafely(() => notifyAdmins({
    type: 'new_review',
    title: 'New review awaiting approval',
    message: reviewAdminMessage(review, user),
    link: '/admin/reviews',
  }))
}
...
export async function createReview(request, response) {
  ...
  await notifyReviewSubmitted(review, request.user)
  return sendSuccess(response, { statusCode: 201, message: 'Review submitted for administrator approval', data: { review } })
}
export async function updateOwnReview(request, response) {
  const review = await Review.findOne({ _id: request.params.id, user: request.user._id })
  if (!review) throw new AppError('Review not found', 404)
  review.rating = request.validatedBody.rating
  review.title = request.validatedBody.title
  review.comment = request.validatedBody.comment
  review.isApproved = false
  await review.save()
  await recalculateProductRating(review.product)
  await review.populate('product', 'name slug image')
  await notifyReviewSubmitted(review, request.user)
  return sendSuccess(response, { message: 'Review updated and submitted for approval', data: { review } })
}
...
export async function moderateReview(request, response) {
  const review = await Review.findById(request.params.id)
  if (!review) throw new AppError('Review not found', 404)
  Object.assign(review, request.validatedBody)
  await review.save()
  await recalculateProductRating(review.product)
  await review.populate([{ path: 'user', select: 'firstName lastName email role isActive' }, { path: 'product', select: 'name slug image' }])
  const approved = review.isApproved && review.isVisible
  await notifySafely(() => notifyUser({
    user: review.user,
    type: 'review_moderated',
    title: approved ? 'Your review was approved' : 'Your review status was updated',
    message: approved
      ? `Your review for ${review.product.name} is now visible on Eshaz Dream World.`
      : `Your review for ${review.product.name} is not currently visible.`,
    link: `/product/${review.product.slug}`,
  }))
  return sendSuccess(response, { message: 'Review moderation updated', data: { review } })
}
