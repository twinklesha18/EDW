...
function MobileMenu({ isOpen, onClose, user, isAuthenticated, cartCount, wishlistCount, onCart, onLogout }) {
  ...
  return <>
    <button type="button" onClick={onClose} className="overlay-enter fixed inset-0 z-50 bg-ink/25 backdrop-blur-sm lg:hidden" aria-label="Close mobile menu overlay" />
    <aside id="mobile-navigation" role="dialog" aria-modal="true" className="drawer-enter fixed inset-y-0 right-0 z-[60] flex h-dvh w-[min(92vw,380px)] flex-col bg-cream shadow-2xl lg:hidden" aria-label="Mobile navigation">
