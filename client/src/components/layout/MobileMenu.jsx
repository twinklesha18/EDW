...
function MobileMenu({ isOpen, onClose, user, isAuthenticated, cartCount, wishlistCount, onCart, onLogout }) {
  ...
  return <AnimatePresence>{isOpen && <>
    ...
    <motion.aside id="mobile-navigation" initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 28, stiffness: 260 }} className="fixed inset-y-0 right-0 z-[60] flex h-dvh w-[min(92vw,380px)] flex-col bg-cream shadow-2xl lg:hidden" aria-label="Mobile navigation">
