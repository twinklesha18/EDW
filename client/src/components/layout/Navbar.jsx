...
function Navbar() {
  ...
  return <header className={`fixed inset-x-0 top-0 z-50 border-b transition-all duration-300 ${isScrolled ? 'border-gold/15 bg-white/90 shadow-[0_8px_30px_-20px_rgba(59,47,54,.45)] backdrop-blur-xl' : 'border-transparent bg-cream/90 backdrop-blur-md'}`}>
    <nav className="section-shell flex h-[4.75rem] items-center justify-between gap-4" aria-label="Main navigation">
      ...
      <div className="flex items-center gap-1 sm:gap-2">
        ...
        {isAuthenticated ? <div ref={accountRef} className="relative hidden lg:block">
          ...
          {accountOpen && <div role="menu" className="menu-enter absolute right-0 z-20 mt-2 w-56 rounded-2xl border border-gold/15 bg-white p-2 shadow-luxury">
    ...
    {isSearchOpen && <div id="navbar-search" className="menu-enter overflow-hidden border-t border-gold/10 bg-white"><div className="section-shell py-4"><SearchBar value={search} onChange={(e) => setSearch(e.target.value)} onSubmit={submitSearch} onClear={() => setSearch('')} autoFocus /></div></div>}
